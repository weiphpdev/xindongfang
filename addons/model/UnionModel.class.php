...
class UnionModel extends Model {
	 ...
	 * @param integer $fid
	 *        	用户ID
	 * @return integer 用户联盟状态，格式为array('unioning'=>1,'unioner'=>1)
	 */
	 ...
	 * @return array 搜索后的数据
	 */
	public function searchUnions($key, $type, $limit = 20, $uid, $gid) {
		// 数据参数不正确
		if ($key === '' || empty ( $uid )) {
			return array ();
		}
