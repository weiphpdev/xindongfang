	...
	drop_menu_list: {
		load: function() {
			...
			$('body').bind('click',function(event){
				...
				var wrap = ['search', 'notice', 'account', 'application'];
				for (var i in wrap) {
			...
			if ($(parentModel).attr('model-node') === 'notice') {
				M.addListener(parentModel, {
					click: function(event) {
						ui.box.load(U('public/Index/messageBox'),null);
						// 老版消息弹出
						// event = event || window.event;
						// var obj = "undefined" != typeof(event.srcElement) ? event.srcElement : event.target;
						//  if ($(obj).attr('rel') !== 'notice-btn' && $(obj).attr('rel') != 'all') {
						// 	return false;
						// }
						...
						// 	if (first) {
						// 		var type = message.whichChangeTab();
						// 		message.changeTab(type);
						// 		first = false;
						// 	}
						// } else {
						// 	if ($(obj).attr('rel') != 'all') {
			...
			} else if ($(parentModel).attr('model-node') === 'search') {
				M.addListener(parentModel, {
					click: function(event) {
						event = event || window.event;
						var obj = "undefined" != typeof(event.srcElement) ? event.srcElement : event.target;
