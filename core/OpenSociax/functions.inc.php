...
function _parse_theme($data) {
	// 如果话题被锁定，则不带链接
	if (! model ( 'FeedTopic' )->where ( array (
			'name' => $data [1] 
	) )->getField ( 'lock' )) {
		return "<a href=" . U ( 'public/Topic/index', array (
				'k' => urlencode ( $data [1] ) 
		) ) . ">" . $data [0] . "</a>";
	} else {
		return $data [0];
...
function _parse_wap_at_by_uname($name) {
	...
	if ($info && $info ['is_active'] && $info ['is_audit'] && $info ['is_init']) {
		return '<a href="' . U ( 'wap/Index/weibo', array (
				'uid' => $info ['uid'] 
		) ) . '" >' . $name [0] . "</a>";
	} else {
		return $name [0];
...
function getVisitorClient() {
	...
	/*
	 * if(preg_match("/android/i", $_SERVER['HTTP_USER_AGENT'])){ return 2; }elseif(preg_match("/iphone/i", $_SERVER['HTTP_USER_AGENT'])){ return 3; }elseif(preg_match("/ipad/i", $_SERVER['HTTP_USER_AGENT'])){ return 3; } elseif(stristr($_SERVER['HTTP_VIA'],"wap") || strpos(strtoupper($_SERVER['HTTP_ACCEPT']),"VND.WAP.WML") > 0){// 先检查是否为wap代理，准确度高 return 1; }elseif(preg_match('/(windows ce)/i', $_SERVER['HTTP_USER_AGENT'])){ return 5; }
	 */
	return '0';
}
	...
	foreach ( $apps as $app ) {
		...
		foreach ( $child_menu as $k => $cm ) {
			...
			if ($k == $app ['app_name'] && $cm ['public'] == 1) {
				...
				$url = U ( 'public/Profile/appprofile', array (
						'appname' => $k 
				) );
...
function isInvite() {
	...
	$result = false;
	if (in_array ( $config ['register_type'], array (
			'open',
			'invite' 
	) )) {
		$result = true;
 ...
 *        	删除关联信息参数，app、module、method
 * @param integer $level
 *        	添加子分类层级，默认为0，则可以添加无限子分类
 * @param integer $times
 *        	用于记录递归层级的次数，默认为1，调用函数时，不需要传入值。
 * @param integer $limit
 *        	分类限制字数。
...
function showTreeCategory($data, $stable, $left, $delParam, $level = 0, $ext = '', $times = 1, $limit = 0) {
	...
	foreach ( $data as $val ) {
		...
		$ext !== '' && $html .= '&nbsp;-&nbsp;<a href="' . U ( 'admin/Public/setCategoryConf', array (
				'cid' => $val ['id'],
				'stable' => $stable 
		) ) . '&' . $ext . '">分类配置</a>';
...
// 获取当前论坛所在城市
function get_city($city = '', $info = array()) {
	if ($city > 0) {
		session ( 'city', $city );
	} elseif ($info ['city'] > 0) {
		session ( 'city', $info ['city'] );
	} elseif (! empty ( $_REQUEST ['city'] )) {
		$_REQUEST ['city'] = intval ( $_REQUEST ['city'] );
		$_REQUEST ['city'] > 0 && session ( 'city', $_REQUEST ['city'] );
	}
	
	$city = intval ( session ( 'city' ) );
	$city == 0 && $city = 1;
	
	return $city;
}
function getMyCity($uid) {
	$list = M ( 'city' )->field ( 'id,manager_uids' )->select ();
	$city = 0;
	foreach ( $list as $vo ) {
		$uids = explode ( ',', $vo ['manager_uids'] );
