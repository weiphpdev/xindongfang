 ...
 */
	...
	class AdminAction extends AdministratorAction {
		...
		public function index() {
			...
			$this->opt ['recommend'] = array (
					'0' => L ( 'PUBLIC_SYSTEMD_NOACCEPT' ),
					'1' => '是',
					'2' => '否' 
			);
		...
		public function editWeiba() {
			...
			$this->pageTab [] = array (
					'title' => '微吧列表',
					'tabHash' => 'index',
					'url' => U ( 'weiba/Admin/index' ) 
			);
			// $this->pageTab[] = array('title'=>'添加微吧','tabHash'=>'addWeiba','url'=>U('weiba/Admin/addWeiba'));
			$this->pageTab [] = array (
					'title' => '微吧分类',
					'tabHash' => 'weibaCate',
					'url' => U ( 'weiba/Admin/weibaCate' ) 
			);
			$this->pageTab [] = array (
					'title' => '编辑微吧',
					'tabHash' => 'editWeiba',
					'url' => U ( 'weiba/Admin/editWeiba', array (
							'weiba_id' => $_GET ['weiba_id'] 
					) ) 
			);
			$this->pageTab [] = array (
					'title' => '帖子列表',
					'tabHash' => 'postList',
					'url' => U ( 'weiba/Admin/postList' ) 
			);
			$this->pageTab [] = array (
					'title' => '帖子回收站',
					'tabHash' => 'postRecycle',
					'url' => U ( 'weiba/Admin/postRecycle' ) 
			);
			// 列表key值 DOACTION表示操作
			$this->pageKeyList = array (
					'weiba_id',
		...
		public function doEditWeiba() {
			...
			if ($res !== false) {
				// 更新帖子的城市
				$city_map ['weiba_id'] = $weiba_id;
				M ( 'weiba_post' )->where ( $city_map )->setField ( 'city', $data ['city'] );
				// 现有超级圈主
				$follow ['level'] = 3;
		...
		public function weibaCate() {
			...
			$this->pageButton [] = array (
					'title' => '添加分类',
					'onclick' => "javascript:location.href='" . U ( 'weiba/Admin/addWeibaCate', array (
							'tabHash' => 'weibaCate' 
					) ) . "';" 
			);
			$this->pageButton [] = array (
			...
			foreach ( $list ['data'] as &$v ) {
				$v ['DOACTION'] = "<a href='" . U ( 'weiba/Admin/editWeibaCate', array (
						'id' => $v ['id'],
						'tabHash' => 'weibaCate' 
				) ) . "'>编辑</a>&nbsp;-&nbsp;<a href='javascript:;' onclick='admin.delWeibaCate(" . $v ['id'] . ")'>删除</a>";
			}
		...
		public function doAddWeibaCate() {
			...
			if ($name) {
				...
				if ($res) {
					$this->assign ( 'jumpUrl', U ( 'weiba/Admin/weibaCate', array (
							'tabHash' => 'weibaCate' 
					) ) );
					$this->success ( '添加成功' );
				} else {
		...
		public function delWeiba() {
			...
			! is_array ( $_POST ['weiba_id'] ) && $_POST ['weiba_id'] = array (
					$_POST ['weiba_id'] 
			);
			$data ['weiba_id'] = array (
					'in',
		...
		public function postList() {
			...
			// 数据的格式化与listKey保持一致
			$listData = D ( 'Weiba', 'weiba' )->getPostList ( 20, array (
					'is_del' => 0 
			) );
			$this->displayList ( $listData );
		}
		...
		public function indexPost() {
			...
			if ($scount < 5) {
				$this->pageButton [] = array (
						'title' => '添加帖子',
						'onclick' => "location.href='" . U ( 'weiba/Admin/newImg' ) . "'" 
				);
			}
			// 数据组装
		...
		public function postRecycle() {
			...
			$this->pageButton [] = array (
					'title' => '还原',
					'onclick' => "admin.recoverPost()" 
			);
			$this->pageButton [] = array (
					'title' => '彻底删除',
					'onclick' => "admin.deletePost()" 
			);
			$this->searchKey = array (
					'post_id',
					'title',
		...
		public function setPost() {
			...
			if (! $result) {
				...
				$config ['post_url'] = '<a href="' . U ( 'weiba/Index/postDetail', array (
						'post_id' => $post_id 
				) ) . '" target="_blank">' . $post_detail ['title'] . '</a>';
				switch ($_POST ['type']) {
					case '1' : // 推荐
					           // 添加积分
						if ($value == 1) {
		...
		public function editPost() {
			...
			$this->pageTab [] = array (
					'title' => '编辑帖子',
					'tabHash' => 'editPost',
					'url' => U ( 'weiba/Admin/editPost', array (
		...
		public function doEditPost() {
			...
			if ($res !== false) {
				...
				$this->assign ( 'jumpUrl', U ( 'weiba/Admin/postList', array (
						'tabHash' => 'postList' 
				) ) );
				$this->success ( L ( 'PUBLIC_SYSTEM_MODIFY_SUCCESS' ) );
			} else {
		...
		public function delPost() {
			...
			! is_array ( $_POST ['post_id'] ) && $_POST ['post_id'] = array (
					$_POST ['post_id'] 
			);
			$data ['post_id'] = array (
					'in',
		...
		public function weibaAdminAuditConfig() {
			...
			$this->opt ['follower_open'] = array (
					'0' => '否',
					'1' => '是' 
			);
			$this->opt ['level_open'] = array (
					'0' => '否',
					'1' => '是' 
			);
			$this->opt ['weiba_post_open'] = array (
					'0' => '否',
					'1' => '是' 
			);
		...
		public function doWeibaAdminAuditConfig() {
			...
			if ($res) {
				$this->assign ( 'jumpUrl', U ( 'weiba/Admin/weibaAdminAuditConfig', array (
						'tabHash' => 'weibaAdminAuditConfig' 
				) ) );
				$this->success ( '保存成功' );
			} else {
				$this->error ( '保存失败' );
		...
		public function weibaAdminAudit() {
			...
			if (! empty ( $_POST ['weiba_name'] )) {
				$maps ['weiba_name'] = array (
						'like',
						'%' . t ( $_POST ['weiba_name'] ) . '%' 
				);
				$map ['weiba_id'] = array (
						'in',
						getSubByKey ( D ( 'weiba' )->where ( $maps )->field ( 'weiba_id' )->findAll (), 'weiba_id' ) 
				);
			}
			$map ['status'] = 0;
			// 数据的格式化与listKey保持一致
		...
		public function doAudit() {
			...
			! is_array ( $_POST ['id'] ) && $_POST ['id'] = array (
					$_POST ['id'] 
			);
			$map ['id'] = array (
					'in',
		...
		public function weibaAuditConfig() {
			...
			$this->opt ['apply_weiba_open'] = array (
					'0' => '否',
					'1' => '是' 
			);
			$this->opt ['follower_open'] = array (
		...
		public function doWeibaAuditConfig() {
			...
			$res = model ( 'Xdata' )->put ( $key, $value );
			if ($res) {
				$this->assign ( 'jumpUrl', U ( 'weiba/Admin/weibaAuditConfig', array (
						'tabHash' => 'weibaAuditConfig' 
				) ) );
				$this->success ( '保存成功' );
			} else {
				$this->error ( '保存失败' );
		...
		public function weibaAudit() {
			...
			foreach ( $listData ['data'] as $k => $v ) {
				$listData ['data'] [$k] ['weiba_name'] = '<a target="_blank" href="' . U ( 'weiba/Index/detail', array (
						'weiba_id' => $v ['weiba_id'] 
				) ) . '">' . $v ['weiba_name'] . '</a>';
		...
		private function _initWeibaListAdminMenu() {
			...
			$this->pageTab [] = array (
					'title' => '申请圈主配置',
					'tabHash' => 'weibaAdminAuditConfig',
					'url' => U ( 'weiba/Admin/weibaAdminAuditConfig' ) 
			);
			$this->pageTab [] = array (
					'title' => '圈主审核',
					'tabHash' => 'weibaAdminAudit',
					'url' => U ( 'weiba/Admin/weibaAdminAudit' ) 
			);
			$this->pageTab [] = array (
					'title' => '申请微吧配置',
					'tabHash' => 'weibaAuditConfig',
					'url' => U ( 'weiba/Admin/weibaAuditConfig' ) 
			);
			$this->pageTab [] = array (
					'title' => '微吧审核',
					'tabHash' => 'weibaAudit',
					'url' => U ( 'weiba/Admin/weibaAudit' ) 
			);
			$this->pageTab [] = array (
					'title' => '首页帖子',
					'tabHash' => 'indexPost',
					'url' => U ( 'weiba/Admin/indexPost' ) 
			);
		}
	}