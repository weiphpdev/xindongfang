...
// 		 $_SESSION['ttt2'] = 123;
		...
		if ($setting ['channel'] == '1') {
			...
			if (! $setting ['channelid']) {
				...
				$map ['feed_channel_link_id'] = array (
						'in',
						$setting ['channelid'] 
				);
				$list = D ( 'Channel', 'channel' )->where ( $map )->order ( 'rand()' )->limit ( 8 )->findAll ();
		...
		if ($setting ['relateduser'] == '1') {
			...
			// 用户组图标
			$icon = getSubByKey ( $_user_recommend ['user_group'], 'user_group_icon', array (
					'user_group_id',
					$_user_recommend ['verified_info'] ['usergroup_id'] 
			) );
			$icon = array_pop ( $icon );
